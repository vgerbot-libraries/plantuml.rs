//! Ported from `net.sourceforge.plantuml.tim.builtin.IsDark`.
...
static SIGNATURE: LazyLock<TFunctionSignature> =
    LazyLock::new(|| TFunctionSignature::new("%is_dark", 1));

/// `%is_dark(color)` — returns true if the color is dark.
///
/// TODO: implement with `HColor`/`HColorSet` once the klimt color crate is ported.
pub struct IsDark;

impl SimpleReturnFunction for IsDark {}
